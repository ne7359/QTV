...
###（8）. 现在可以使用：https://服务器 ip:4000 登录了，默认账号和密码是：admin/password
...
（9）. 记住 there are no members on this network - users are invited to join 后面的网络 id，后续方便虚拟局域网的其他电脑加入

（10）. 下载 / root 目录下生成的 “planet” 文件，来替换需要组网客户端里的 “planet” 文件

（11）. 其他需要加入虚拟局域网的电脑正常安装客户端，各系统的客户端下载地址：https://www.zerotier.com/download/

（12）. 使用刚才下载的 “planet” 文件替换其他电脑 “ZeroTier-One” 安装目录下的 “planet” 文件，并重启系统

（13）. 在设备上执行 “zerotier-cli join 你的 planet 服务器网络 ID” 加入刚组建虚拟局域网
```
zerotier-cli join xxxxxxxxxx
```
如果一切正常的话，会显示：200 join OK 的提示

（14）在其他设备上执行 zerotier-cli listpeers 命令可以看到你架设成功的 planet 服务器，注意这里的 planet 服务器是不显示 ip 的。具体见下图
